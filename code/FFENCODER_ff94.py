...
from protein_info_finder import torsion_info
from tor_nonb_calc_ff94 import tor_calc
from tor_nonb_calc_ff94 import nonb_calc
from ref_des_build import ref_des_ff94
...
newfile1 = open(direct1+'descriptort_amber_ff94.csv','w')
newfile2 = open(direct1+'descriptorn_amber_ff94.csv','w')
filenames = [i for i in os.listdir(direct1) if '.pdb' in i and '.swp' not in i and 'inserted' not in i]


[dictor, dicnon] = ref_des_ff94()
t = pd.read_csv('template/descriptort_ff94.csv')
n = pd.read_csv('template/descriptorn_ff94.csv')
...
for pdbname in filenames:
    ...
    pro_tor = torsion_info(pro_info[0], pro_info[1])
