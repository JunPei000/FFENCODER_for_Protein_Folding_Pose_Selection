...
direct2 = PATH OF THE FINAL DESCRIPTOR FILES
direct3 = 'important_f_amber_ff94/'
impo = open(direct3+'amber_ff94_with_oop_500.txt','r')
imp = impo.readlines()
important_f = []
for line in imp:
    m = line.split()
...
print (important_f)
feature_to_drop = []
for column in [str(i) for i in range(0,19480)]:
    if column != '19479' and column not in important_f:
...
x_train = train_data.drop(['19479'], axis=1).iloc[:,1:]
y_train = train_data['19479']
...
for tfile in testfiles:
    ...
    for row in temp.index:
        if float(temp.loc[row, '19479']) != 0 and float(temp.loc[row, '19479']) != 1:
            print (tfile, row, temp.loc[row, '19479'])
if 'Unnamed: 0.1' in test_data.columns:
    test_data = test_data.drop(['Unnamed: 0.1'], axis=1)
x_test = test_data.drop(['19479'], axis=1).iloc[:,1:]
y_test = test_data['19479']
print(x_test)
print(y_test)
...
for tfile in testfiles:
    ...
    x_test = test_data.drop(['19479','Unnamed: 0.1'], axis=1).astype('float').drop(feature_to_drop,axis=1)
    print (x_test)
    names = []
    scores = []
